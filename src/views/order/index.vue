<template>
  <v-container fluid>
    <div>
      <h2 class="pa-2 text-h4 font-weight-bold main--text">Оформить заказ</h2>
    </div>
    <v-row class="mt-6" outlined>
      <v-col cols="6">
        <v-card>
          <div class="pa-4">
            <h3 class="text-h5 font-weight-bold main--text">Личные данные</h3>
        ...
        <v-card class="mt-4">
          <div class="pa-4">
            <h3 class="text-h5 font-weight-bold main--text">Ваш заказ</h3>
            <v-list-item class="mt-5 pa-0" v-for="(product, index) in products" :key="index">
              <v-list-item-content>
                <div class="d-flex justify-space-between align-center">
                  ...
                  <div>
                    <p class="primary--text font-weight-bold">{{ product.price }} сум</p>
        ...
        <v-card class="mt-6">
          <div class="pa-4">
            <h3 class="text-h5 font-weight-bold main--text">Тип доставки</h3>
            <v-radio-group row v-model="form.delivery_type">
              <v-radio label="Доставка" value="Доставка" class="grayBack rounded-lg pa-2"
                style="width: 45%"></v-radio>
              <v-radio label="Самовывоз" value="Самовывоз" class="grayBack rounded-lg pa-2"
                style="width: 45%">
              </v-radio>
            </v-radio-group>
          </div>
        ...
        <v-card class="mt-6">
          <div class="pa-4">
            <h3 class="text-h5 font-weight-bold main--text">Выберите филиал</h3>
        ...
        <v-card class="mt-6">
          <div class="pa-4">
            <h3 class="text-h5 font-weight-bold main--text">Способ оплаты</h3>
            <v-radio-group v-model="form.payment_type" row>
              <v-radio label="Наличный" value="Наличный" class="grayBack rounded-lg pa-2" style="width: 45%"></v-radio>
          ...
          <div class="mt-8">
            <v-textarea v-model="form.comment" solo name="input-7-4" label="comment"></v-textarea>
          </div>
      </v-col>
      <v-col cols="6">
        <v-card class="pa-4">
          <h3 class="text-h5 font-weight-bold main--text mb-4">Итого</h3>
          <v-row class="pa-3" v-for="(product, index) in products" :key="index">
            <v-col cols="6 pa-0">
              <p class="grayDark--text">Стоимость товаров</p>
            </v-col>
            <v-col cols="6 pa-0">
              <p class="d-flex justify-end main--text font-weight-bold">{{ product.price }} сум</p>
            </v-col>
          </v-row>
          <v-divider class="my-4"></v-divider>
          <v-row>
            <v-col>
              <p class="main--text font-weight-bold">Обшая сумма</p>
            </v-col>
            <v-col>
              <span class="primary--text font-weight-bold d-flex justify-end">{{ totalPrice }} сум</span>
            </v-col>
          </v-row>
          <v-card-actions class="pa-0 mt-6">
            <v-btn @click="createOrder" text color="white" class="primary py-6 rounded-lg"
